<?php

declare(strict_types=1);

use Sumire\Database;
use Sumire\Tests\Fixtures\Post;
use Sumire\Tests\Fixtures\PostStatus;
use Sumire\Tests\Fixtures\User;

require __DIR__ . '/../vendor/autoload.php';

function assert_true(bool $condition, string $message): void
{
    if (!$condition) {
        throw new RuntimeException($message);
    }
}

$driver = getenv('SUMIRE_DRIVER') ?: '';
$dsn = getenv('SUMIRE_DSN') ?: '';
$dbUser = getenv('SUMIRE_USER') ?: null;
$password = getenv('SUMIRE_PASSWORD') ?: null;

if (!in_array($driver, ['mysql', 'pgsql'], true) || $dsn === '') {
    fwrite(STDERR, "Set SUMIRE_DRIVER=mysql|pgsql, SUMIRE_DSN, SUMIRE_USER, and SUMIRE_PASSWORD.\n");
    exit(2);
}

$database = Database::connect(new PDO($dsn, $dbUser, $password));
$connection = $database->connection();

$connection->execute('DROP TABLE IF EXISTS posts');
$connection->execute('DROP TABLE IF EXISTS users');
$database->createTable(User::class);
$database->createTable(Post::class);

$repository = $database->repository(User::class);

$user = new User('Ada Lovelace', 'ada.integration@example.com');
$database->persist($user);

assert_true($user->id() === 1, sprintf('%s should assign generated id.', $driver));

$found = $repository->find($user->id());

assert_true($found instanceof User, sprintf('%s should find inserted user.', $driver));
assert_true($found->active() === true, sprintf('%s should hydrate true boolean.', $driver));

$found->deactivate();
$database->persist($found);

$inactive = $repository->firstBy(['active' => false]);

assert_true($inactive instanceof User, sprintf('%s should query false boolean criteria.', $driver));
assert_true($inactive->active() === false, sprintf('%s should hydrate false boolean.', $driver));

$database->transaction(function (Database $outer) use ($repository): void {
    $outer->persist(new User('Outer Commit', 'outer.integration@example.com'));

    try {
        $outer->transaction(function (Database $inner): void {
            $inner->persist(new User('Inner Rollback', 'inner.rollback.integration@example.com'));

            throw new PDOException('inner rollback');
        });
    } catch (PDOException $exception) {
        assert_true($exception->getMessage() === 'inner rollback', 'Nested transaction should rethrow callback exceptions.');
    }

    assert_true(
        $repository->firstBy(['email' => 'inner.rollback.integration@example.com']) === null,
        'Nested rollback should roll back only to the savepoint.',
    );
});

assert_true(
    $repository->firstBy(['email' => 'outer.integration@example.com']) instanceof User,
    sprintf('%s should commit the outer transaction after an inner rollback.', $driver),
);
assert_true(
    $repository->firstBy(['email' => 'inner.rollback.integration@example.com']) === null,
    sprintf('%s should not persist rows rolled back to a savepoint.', $driver),
);

$createdAt = new DateTimeImmutable('2026-07-15 12:34:56');
$post = new Post('Schema Types', PostStatus::Draft, ['driver' => $driver], $createdAt);
$database->persist($post);

$storedPost = $database->find(Post::class, $post->id());

assert_true($storedPost instanceof Post, sprintf('%s should store an entity in a generated table.', $driver));
assert_true($storedPost->status() === PostStatus::Draft, sprintf('%s should infer backed enum columns.', $driver));
assert_true($storedPost->metadata() === ['driver' => $driver], sprintf('%s should infer JSON columns.', $driver));
assert_true(
    $storedPost->createdAt()->format('Y-m-d H:i:s') === $createdAt->format('Y-m-d H:i:s'),
    sprintf('%s should infer datetime columns.', $driver),
);

$database->remove($inactive);

assert_true($repository->find($user->id()) === null, sprintf('%s should remove entity.', $driver));

echo sprintf("%s integration test passed.\n", $driver);
